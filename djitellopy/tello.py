...
class Tello:
    ...
    def send_velocity_enu(self, vel_enu, heading):
        ...
        R = RBI(self.heading)
        V_err_xyz = R.dot(V_err_enu) # Transform the error speed vector from ENU frame to the local frame of the drone uising the rotational matrix of psi angle
        err_heading = norm_ang(norm_ang(heading) - self.heading) # compute the error in the front direction
    ...
    def fly_to_enu(self,position_enu, heading=None):
        ...
        vel_enu = pos_error*0.75 - self.velocity_enu
