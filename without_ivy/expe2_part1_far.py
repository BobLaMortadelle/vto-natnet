        ...
        if (time.time() - start_light > 0.05):
            ...
            try:
                if(fire.is_set()):
                    ...
                    tello.send_expansion_command("mled s p {letter}".format(letter = randletter))
                    ok.clear()
                    standby.set()
                    
                elif(handover_text.is_set()):
                    tello.send_expansion_command("mled l p 2.5 Helicopter ")
                    handover_text.clear()
                    standby.set()
...
def choose_animation(choice):
    ...
    if choice == 1:
        ...
        logMessage = "ok displayed".format(letter = randletter)
        ok.set()
                
    elif choice == 8:
        logMessage = "heliocpter text displayed"
        handover_text.set()
...
def display_button():
    ...
    pygame_handler = logging.FileHandler('expe2_far_{number}.log'.format(number = text), mode="w")
...
def flight_routine(swarm, voliere):
    ...
    drone_position_handler = logging.FileHandler('Drone_positions_participant_far_{number}.log'.format(number = userNb), mode="w")
    ...
    user_position_handler = logging.FileHandler('User_positions_participant_far_{number}.log'.format(number = userNb), mode="w")
    ...
    bracelet_position_handler = logging.FileHandler('Bracelet_positions_participant_far_{number}.log'.format(number = userNb), mode="w")
