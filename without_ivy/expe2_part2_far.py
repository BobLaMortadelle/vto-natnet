...
def display_button():
    ...
    pygame_handler = logging.FileHandler('expe2_far_{nombre}.log'.format(nombre = text), mode="a")
...
def flight_routine(swarm, voliere):
    ...
    drone_position_handler = logging.FileHandler('Drone_positions_participant_far_{number}.log'.format(number = userNb), mode="a")
    ...
    user_position_handler = logging.FileHandler('User_positions_participant_far_{number}.log'.format(number = userNb), mode="a")
    ...
    bracelet_position_handler = logging.FileHandler('Bracelet_positions_participant_far_{number}.log'.format(number = userNb), mode="a")
    ...
    try:
                ...
                if(wPCounter < lengthWPs-1):
                    ...
                    if(dist <= epsilonDist and (angle %(2*math.pi)<= minepsilonAngle or angle%(2*math.pi) >= maxepsilonAngle)):                            
                        ...
                        if(lastSequence != sequenceCounter):
                            if sequenceCounter == 0  and not firstanimation.is_set(): 
                                handover_text.clear() 
                                randomIndex = random.randint(0, len(animations)-1)
                                choose_animation(animations[randomIndex])   
                                animations.pop(randomIndex)
                                firstanimation.set()
                            elif sequenceCounter == 1  and not secondanimation.is_set():  
                                fire.clear()
                                survivor.clear()
                                ...
                                secondanimation.set()
                            elif sequenceCounter == 2  and not thirdanimation.is_set():  
                                fire.clear()
                                survivor.clear()
                                hazardous_material.clear()
                                map.clear()
                                ...
                                thirdanimation.set()
                            elif sequenceCounter == 3  and not fourthanimation.is_set():  
                                fire.clear()
                                survivor.clear()
                                hazardous_material.clear()
                                map.clear()
                                ...
                                choose_animation(animations[randomIndex])   
                                animations.pop(randomIndex)
                                thirdanimation.clear()
                                fourthanimation.set()
                            elif sequenceCounter == 4  and not fifthanimation.is_set():  
                                fire.clear()
                                survivor.clear()
                                hazardous_material.clear()
                                map.clear()
                                ...
                                choose_animation(animations[randomIndex])   
                                animations.pop(randomIndex)
                                fourthanimation.clear()
                                fifthanimation.set()
                            elif sequenceCounter == 5  and not sixthanimation.is_set():  
                                fire.clear()
                                survivor.clear()
                                hazardous_material.clear()
                                map.clear()
        ...
        swarm.land()
        swarm.turn_motor_on()
        # with open('drone_positions.csv', 'a', newline='') as positionsFile:
        #     writerDrone = csv.writer(positionsFile)
        #     writerDrone.writerows(positionsTello)
        # with open('user_positions.csv', 'a', newline='') as positionsFile:
